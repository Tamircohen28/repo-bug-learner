/*
rule = CalendarNowWithoutExplicitTimezone
*/
...
class CalendarAvailabilityService {
  ...
  def computeWindowStart(): DateTime =
    DateTime.now() /* assert: CalendarNowWithoutExplicitTimezone.calendar-now-without-explicit-timezone*/

  // BAD: bare LocalDateTime.now()
  def computeSlotStart(): LocalDateTime =
    LocalDateTime.now() /* assert: CalendarNowWithoutExplicitTimezone.calendar-now-without-explicit-timezone*/

  // BAD: bare LocalDate.now()
  def today(): LocalDate =
    LocalDate.now() /* assert: CalendarNowWithoutExplicitTimezone.calendar-now-without-explicit-timezone*/
